							...
							if ($relation){
								...
								if ($contract){
									...
									if($contractCodeVar === $contractCode and $contractCreatedAt === $contractDate){
										...
										Session::flash('danger', 'Номер или дата договора не совпадают');
									}
								}else{
									Session::flash('danger', 'Контракт не существует');
								}
							}else{
								Session::flash('danger', 'Связь между клиентом и дебитором не найдена');
							}
						}else{
